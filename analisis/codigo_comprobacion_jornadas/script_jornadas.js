...
function comprobacionCompleta() {
  generarHojaRutasPresencias();
  procesarDatosCompletos();
}

// Script que orquesta las operaciones para la creación de las hojas para la casación de hojas de ruta, trabajadores y presencia, y los fichajes sin rutas
...
function _extraerIdTrabajadorEnHojasOrigen(spreadsheet) {
  ...
  if (titularSheet) {
    ...
    if (idColumnIndex === -1) {
      ...
      titularSheet
        .getRange(2, idColumnIndex + 1, idData.length, 1)
        .setValues(idData);
    }
  }
...
function _procesarYGenerarHojaDeSalida(spreadsheet) {
  ...
  trabajadoresData
    .slice(1)
    .forEach((row) =>
      trabajadoresMap.set(Number(String(row[0]).trim()), {
        apellido1: row[4],
        apellido2: row[5],
        nombre: row[6],
        puesto: row[22],
  ...
  fichajesData.slice(1).forEach((row) => {
    ...
    if (workerId) {
      ...
      fichajesMap
        .get(workerId)
        .push({
          time: _corregirHusoHorarioExacto(row[0]),
          type: row[7],
          rawRow: row,
        });
    }
  ...
  if (titularesSheet) {
    ...
    titularesData.slice(1).forEach((row) => {
      ...
      const { entryTime, exitTime } = _getFichajeTimes(
        fichajesMap,
        workerId,
        startTime,
        endTime,
  ...
  if (auxiliaresSheet) {
    ...
    titularesData
      .slice(1)
      .forEach((row) =>
        titularesMap.set(row[0], {
          team: row[2],
          turno: row[4],
          startTime: _corregirHusoHorarioExacto(row[16]),
          endTime: _corregirHusoHorarioExacto(row[20]),
          associatedParts: row[30],
          generalDate: row[3],
          service: row[1],
          vehicle: row[6],
          kms: row[14],
...
function _getFichajeTimes(
  ...
  const entryWindowStart = startTime
  ...
  for (const fichaje of workerFichajes) {
    ...
    if (
      fichaje.type === "1 - Entrada" &&
      entryWindowStart &&
      fichaje.time >= entryWindowStart &&
      fichaje.time <= endTime
    ) {
    ...
    if (
      fichaje.type === "2 - Salida" &&
      exitWindowEnd &&
      fichaje.time >= startTime &&
      fichaje.time <= exitWindowEnd
    ) {
...
function _ajustarHorariosDePresencia() {
  ...
  if (!sheet) {
    Logger.log(
      "La hoja 'rutas-presencias' no existe. Por favor, ejecuta primero el script de casación.",
    );
    return;
  }
  ...
  if (Object.values(colIndices).some((idx) => idx === -1)) {
    Logger.log(
      "Una o más columnas necesarias no se encontraron en la hoja 'rutas-presencias'.",
    );
    return;
  }
  ...
  while (i < rows.length) {
    ...
    if (group.length > 1) {
      ...
      // Última hoja de ruta
      if (fichajeSalida) {
        group[group.length - 1][colIndices.ENTRADA] =
          group[group.length - 1][colIndices.FECHA_HORA_INI];
...
function _identificarRutasDuplicadas() {
  ...
  const rutasDuplicadasCol =
    headers.indexOf("rutas duplicadas") === -1
      ? headers.length
      : headers.indexOf("rutas duplicadas");
  const rutasARevisarCol =
    headers.indexOf("rutas a revisar") === -1
      ? headers.length + (rutasDuplicadasCol === headers.length ? 1 : 0)
      : headers.indexOf("rutas a revisar");
  ...
  for (const key in grupos) {
    ...
    if (grupo.length > 1) {
      ...
      grupo.forEach(
        (item) => (updatedRows[item.index][rutasDuplicadasCol] = true),
      );

      // Lógica para marcar "rutas a revisar"
      const partesAsociadosCount0 = grupo.filter(
        (item) => item.row[PARTES_ASOCIADOS_COL] === 0,
      ).length;
      const equiposUnicos = new Set(
        grupo.map((item) => item.row[EQUIPO_TRABAJO_COL]),
      );

      let revisar = true;
      if (grupo.length === 2 && partesAsociadosCount0 > 0) {
        // Caso 1
        revisar = false;
      } else if (equiposUnicos.size === 1) {
        // Caso 2
        revisar = false;
      } else if (equiposUnicos.size === 2 && partesAsociadosCount0 > 0) {
        // Caso 3
        revisar = false;
      }

      grupo.forEach(
        (item) => (updatedRows[item.index][rutasARevisarCol] = revisar),
      );
    }
  }
...
function _generarHojaPresenciasSinRutas(spreadsheet, fichajesAsociados) {
  ...
  fichajesNoAsociados.forEach((row) => {
    const workerId = Number(String(row[4]).trim());
    const fichajeTime = _corregirHusoHorarioExacto(row[0]);
    const dateKey = fichajeTime
      ? Utilities.formatDate(fichajeTime, "GMT", "yyyy-MM-dd")
      : null;

    if (workerId && dateKey) {
      const groupKey = `${workerId}-${dateKey}`;
      if (!fichajesAgrupados.has(groupKey)) {
        fichajesAgrupados.set(groupKey, {
          entries: [],
          exits: [],
          workerId: workerId,
        });
...
function procesarDatosCompletos() {
  ...
  try {
    ...
    Logger.log(
      "Iniciando Paso 2: Búsqueda y unión de datos de 'trabajadores'...",
    );
    ...
    Browser.msgBox(
      "Proceso Completado",
      'Las hojas "rp-unicas" y "resumen-puestos" han sido creadas/actualizadas con éxito.',
      Browser.Buttons.OK,
    );
  } catch (e) {
    ...
    Browser.msgBox(
      "Error",
      "Ocurrió un error durante el proceso: " + e.message,
      Browser.Buttons.OK,
    );
  }
}
...
function _extraerRegistrosUnicos(ss) {
  ...
  if (!sheetRutas) {
    Browser.msgBox(
      "Error",
      `No se encontró la hoja de origen: "${SOURCE_SHEET_NAME}". Asegúrate de que el nombre sea correcto.`,
      Browser.Buttons.OK,
    );
    return null;
  }
  ...
  if (allData.length <= 1) {
    Logger.log(
      "La hoja " +
        SOURCE_SHEET_NAME +
        " está vacía o solo contiene la cabecera.",
    );
    return null;
  }
  ...
  for (const key in COL_INDEXES) {
    if (COL_INDEXES[key] === -1) {
      Browser.msgBox(
        "Error",
        `No se encontró la columna requerida: "${key.replace(/([A-Z])/g, " $1").trim()}".`,
        Browser.Buttons.OK,
      );
      return null;
    }
...
function _agregarPuestosYEqual(ss, sheetUnicas) {
  ...
  if (!sheetTrabajadores) {
    Browser.msgBox(
      "Error",
      `No se encontró la hoja de trabajadores: "${WORKERS_SHEET_NAME}". Asegúrate de que el nombre sea correcto.`,
      Browser.Buttons.OK,
    );
    return;
  }
  ...
  if (CODE_INDEX === -1 || PUESTO_INDEX === -1 || EQUAL_INDEX === -1) {
    Browser.msgBox(
      "Error",
      'Faltan columnas en la hoja "trabajadores". Se requiere "Código", "Puesto Incorpora" y "Equal".',
      Browser.Buttons.OK,
    );
    return;
  }
  ...
  const ID_TRABAJADOR_INDEX_RP = _getHeaderIndex(
    "id trabajador",
    rpUnicasHeaders,
  );
  ...
  if (
    ID_TRABAJADOR_INDEX_RP === -1 ||
    PUESTO_COL_INDEX === -1 ||
    EQUAL_COL_INDEX === -1
  ) {
    Browser.msgBox(
      "Error",
      'Falta la columna "id trabajador" o las columnas nuevas en la hoja "rp-unicas".',
      Browser.Buttons.OK,
    );
    return;
  }
  ...
  sheetUnicas
    .getRange(1, 1, updatedData.length, updatedData[0].length)
...
function _crearResumenPuestos(ss, sheetUnicas) {
  ...
  if (
    PUESTO_INDEX === -1 ||
    EQUAL_INDEX === -1 ||
    PARTES_ASOCIADOS_INDEX === -1
  ) {
    Browser.msgBox(
      "Error",
      'Faltan las columnas requeridas ("Puesto Incorpora", "Equal" o "partes asociados") en "rp-unicas".',
      Browser.Buttons.OK,
    );
    return;
  }
  ...
  const summaryOutput = [
    [
      "Puesto Incorpora",
      "Equal",
      "Conteo de Registros Únicos (Partes Asociados != 0)",
    ],
  ];
...
function _getHeaderIndex(headerName, headers) {
  return headers.findIndex(
    (h) =>
      h.toString().trim().toLowerCase() === headerName.trim().toLowerCase(),
  );
}
